"""Grad-CAM from the final ResNet feature map using MyTorch autograd."""
...
def grad_cam(model, inputs, head="steering"):
    ...
    try:
        ...
        features = mt.Tensor(model.forward_features(inputs).realize_cached_data(),
        ...
        return mt.Tensor(heatmap, device=features.device, requires_grad=False).numpy()
    finally:
...
def main():
    ...
    policy = AutoDrivePolicy(args.config, device=mt.cuda(0) if args.device == "cuda" else mt.cpu())
    with Image.open(args.image) as image:
        frame = np.asarray(image.convert("RGB"))
    p = policy.preprocessing
    values = preprocess_rgb_frame(frame, p["image_size"], p["mean"], p["std"])
    inputs = mt.Tensor(values[None], device=policy.device, requires_grad=True)
