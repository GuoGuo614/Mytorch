...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        return NDArray.make(
            new_shape, NDArray.compact_strides(new_shape), self._device,
    ...
    def broadcast_to(self, new_shape):
        ...
        for i in range(len(self._shape)):
            if self._shape[i] != new_shape[i]:
                new_strides[i] = 0
        return NDArray.make(
            new_shape, tuple(new_strides), self._device, self._handle,
            self._offset
        )
        ### END YOUR SOLUTION
    ...
    def __getitem__(self, idxs):
        ...
        offset = self._offset + sum(
            sl.start * st for sl, st in zip(idxs, self._strides)
        )
