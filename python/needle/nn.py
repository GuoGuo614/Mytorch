...
class Conv(Module):
    ...
    Uses same padding by default, or an explicit integer padding value
    No grouped convolution or dilation
    Only supports square kernels
    """
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, bias=True,
                 device=None, dtype="float32", padding=None):
        super().__init__()
...
class Conv2d(Conv):
    """PyTorch-style convolution wrapper whose default padding is zero."""

    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding=0, bias=True, device=None, dtype="float32"):
        super().__init__(
            in_channels, out_channels, kernel_size, stride=stride, bias=bias,
            device=device, dtype=dtype, padding=padding
        )


class MaxPool2d(Module):
    """Two-dimensional max pooling for NCHW inputs."""
...
class AvgPool2d(Module):
    """Two-dimensional average pooling for NCHW inputs."""
...
class LogSoftmax(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        return ops.logsoftmax(x, self.axis)
