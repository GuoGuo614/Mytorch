...
class EWisePow(TensorOp):
    ...
    def compute(self, a: NDArray, b: NDArray) -> NDArray:
        return get_array_module(a).power(a, b)
...
class PowerScalar(TensorOp):
    ...
    def compute(self, a: NDArray) -> NDArray:
        return get_array_module(a).power(a, self.scalar)
...
class Reshape(TensorOp):
    ...
    def compute(self, a):
        result = get_array_module(a).reshape(a, self.shape)
...
class BroadcastTo(TensorOp):
    ...
    def compute(self, a):
        reuslt = get_array_module(a).broadcast_to(a, shape=self.shape)
...
class Summation(TensorOp):
    ...
    def compute(self, a):
        result = get_array_module(a).sum(a, axis=self.axes)
        return result.astype(a.dtype)
...
class MatMul(TensorOp):
    def compute(self, a, b):
        ...
        target_dtype = a.dtype if a.dtype == b.dtype else xp.result_type(a.dtype, b.dtype)
...
class Log(TensorOp):
    def compute(self, a):
        return get_array_module(a).log(a)
...
class Exp(TensorOp):
    def compute(self, a):
        return get_array_module(a).exp(a)
...
class ReLU(TensorOp):
    def compute(self, a):
        return get_array_module(a).maximum(0, a)
