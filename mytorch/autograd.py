...
def _check_same_device(values):
    ...
    if devices and any(device != devices[0] for device in devices[1:]):
        names = ", ".join(str(device) for device in devices)
        raise ValueError(f"device mismatch: operation received tensors on {names}")
...
class Tensor(Value):
        ...
        **kwargs
        ...
        if isinstance(array, Tensor):
            ...
            if device == array.device and dtype == array.dtype:
                ...
                cached_data = to_device(
                    array.realize_cached_data(), device, dtype=dtype
                )
        else:
            device = device if device else device_of(array)
    ...
    def _array_from_numpy(numpy_array, device, dtype):
        return to_device(numpy_array, device, dtype=dtype)
    ...
    def data(self, value):
        ...
        if value.device != self.device:
            raise ValueError(
                f"device mismatch: cannot assign {value.device} data to {self.device} tensor"
            )
    ...
    @property
    def device(self):
        return device_of(self.realize_cached_data())
    ...
    def numpy(self):
        return asnumpy(self.realize_cached_data())
